...
exports.getVehiculosEstado = async (req, res) => {
  try {
    ...
    res.status(500).json({
      message:
        err.message ||
        "Ocurrió un error al obtener los estados de los vehículos",
    });
  }
};
...
exports.viewVehicle = async (req, res) => {
  ...
  try {
    ...
    if (!vehiculo) {
      return res
        .status(404)
        .json({ message: `No se encontró el vehículo con el id ${id}` });
    }
...
exports.getKilometrajeHistorial = async (req, res) => {
  ...
  try {
    ...
    const kilometrajeHistorial = reservas.rows.map((reserva) => {
      const bitacora = bitacoras.find(
        (b) => String(b.folioReserva) === String(reserva.folio),
      );
      return {
        folio: reserva.folio,
        ...
        kilometraje_usado: bitacora
          ? bitacora.kilometraje_recibido - reserva.kms_salida
          : null,
        releasedBy_name:
          bitacora && bitacora.releasedBy
            ? usuariosMap[bitacora.releasedBy]
            : null,
        receivedBy_name:
          bitacora && bitacora.receivedBy
            ? usuariosMap[bitacora.receivedBy]
            : null,
      };
    });
    ...
    console.error(
      "Error al obtener el historial de kilometraje:",
      error.message,
    );
    res.status(500).json({
